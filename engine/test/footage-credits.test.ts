...
test("footage attribution survives publishing and cannot be silently truncated",async()=>{
  ...
  const credit=await blobs.put(new TextEncoder().encode(JSON.stringify([{credit:"Footage by Example",source_url:"https://example.com/clip",license_url:"https://example.com/license"}])),{role:"footage_credits"});
  const inputs={
