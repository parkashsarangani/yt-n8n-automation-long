...
const schema = JSON.parse(readFileSync(new URL("../schemas/visual_plan/2.0.0.json", import.meta.url), "utf8"));
...
test("planner v13 opts into explanation visual_plan 2.0", () => {
  assert.equal(agent.version, "13");
  assert.equal(agent.produces_version, "2.0.0");
