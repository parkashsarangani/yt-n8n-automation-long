...
test("editor package uploads the draft and records a beat per scene", async () => {
  ...
  const prompt = await blobs.put(new TextEncoder().encode('{"status":"needs_editor_replacement","image_prompt":"A candid scene"}'), {role:"thumbnail_prompt",media_type:"application/json"});
  const artwork = await blobs.put(new Uint8Array([8,9]), {role:"thumbnail_artwork",media_type:"image/jpeg"});
  ...
  const inputs = {
    ...
    thumbnail: { payload: { thumbnail_uri: thumb.uri, media_type: "image/png", background: "gradient" }, blobs:[thumb,prompt,artwork] } as Artifact,
  };
  ...
  assert.deepEqual(files.map((f) => f.name).sort(), ["captions.srt", "credits.json", "draft.mp4", "package.md", "thumbnail-artwork.jpg", "thumbnail-prompt.json", "thumbnail.png"]);
  assert.deepEqual(await drive.downloadFile(files.find(f=>f.name==="thumbnail-artwork.jpg")!.id),await blobs.get(artwork.uri));
  assert.deepEqual(await drive.downloadFile(files.find(f=>f.name==="thumbnail-prompt.json")!.id),await blobs.get(prompt.uri));
  ...
  assert.equal((await drive.listFiles(payload.drive_folder_id)).length,7);
});
