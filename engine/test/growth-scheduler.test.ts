...
function candidate(overall = 0.8): DiscoveryCandidate {
  return {
    brief: "A dismissed mechanic warns everyone that a machine is about to fail, then becomes the only person who can repair it.",
    genre: "drama",
...
test("viability floor rejects weak packages before production spend", () => {
  ...
  assert.equal(viableCandidate(weak), false);
  assert.equal(candidateOverallScore(candidate(0.91)), 0.91);
});

test("package seed remains valid JSON inside intent's 2000-char contract", () => {
  const c = candidate();
  c.evidence = "e".repeat(1000);
  const brief = briefWithPackageSeed(c);
  assert.ok(brief.length <= 2000);
  assert.match(brief, /^RFC0009_PACKAGE_JSON:/);
  const decoded = JSON.parse(brief.slice("RFC0009_PACKAGE_JSON:".length));
  assert.equal(decoded.genre, "drama");
  assert.match(decoded.opening_line, /belt twice/);
});

test("only watchability creative failures authorize topic failover", () => {
  const creative = { status: "blocked", failures: [{ node_id: "watchability_release", error: "watchability release blocked (ABANDON_TOPIC: weak premise)" }] } as any;
  const infrastructure = { status: "blocked", failures: [{ node_id: "render", error: "renderer unavailable" }] } as any;
  assert.equal(creativeFailure(creative), true);
