...
test("story@1.2.0 is an additive minor bump: 1.0.0 artifacts stay valid", async () => {
  const registry = await SchemaRegistry.load(path.join(ROOT, "schemas"));

  // The registry resolves to the newest active version...
  assert.equal(registry.resolveVersion("story"), "1.2.0");
  ...
  // flow — it does not touch the payload shape at all.
  assert.doesNotThrow(() => registry.validate("story", "1.2.0", STORY));
});
