...
test("loads the real project schemas", async () => {
  ...
  for (const id of ["intent", "story", "script", "voice", "rendered_video", "thumbnail", "seo_metadata", "qa_report", "published_episode"]) {
  ...
  assert.equal(reg.resolveVersion("intent"), "2.2.0");
});
