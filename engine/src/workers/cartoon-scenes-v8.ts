...
function assertForegroundCoverage(scripts: ScriptScene[], entries: CompiledEntry[]): void {
  ...
  ordered.forEach((scene, index) => {
    ...
    if (propsMatch(prop?.type ?? "", central.prop)) coveredThirds.add(thirdForIndex(index, ordered.length));
  });
  ...
  if (coveredThirds.size < 2) {
    throw new Error(
      `cartoon_scene_compiler@8 foreground prop gate failed: central object "${central.prop}" must render as a foreground prop in at least two of the opening, middle, and payoff/final thirds; only found ${[...coveredThirds].join(", ") || "none"}`,
    );
  }
}
