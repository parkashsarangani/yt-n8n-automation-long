...
export function makeThumbnailWorker(opts:ThumbnailWorkerOptions={}):WorkerDef {
  return {
    name:"thumbnail",kind:"worker",version:opts.version??"9",
    ...
    async execute(inputs,ctx) {
      ...
      const blobs:BlobRef[]=[];
      const attempts:Array<{prompt:string; outcome:string}>=[];
      ...
      if(prompt && ctx.media.images) for(let attempt=0;attempt<2;attempt++){
        ...
        try {
          ...
          attempts.push({prompt:sent,outcome:"generation or artwork rendering failed"});
          ctx.logger.warn("thumbnail artwork attempt failed; bounded retry or editor replacement required");
        }
      }
