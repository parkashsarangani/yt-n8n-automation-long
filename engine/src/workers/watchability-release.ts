 ...
 * VidGenService owns retry / best-of-N / next-candidate selection; this worker
 * only decides whether the current script clears the creative bar.
 */
...
export function assessWatchability(payload: unknown): {
  ...
  const average = values.length === Object.keys(WATCHABILITY_THRESHOLDS).length
    ...
    : 0;
  if (average < WATCHABILITY_AVERAGE_THRESHOLD) {
    failures.push(`average=${average.toFixed(3)} (requires ${WATCHABILITY_AVERAGE_THRESHOLD.toFixed(2)})`);
  }
  ...
  const abandonReason = typeof report.abandon_reason === "string" && report.abandon_reason.trim()
    ...
    : abandonRecommended
      ? "package/first-30/youtube-fit is materially below the viable floor"
      : "";

  return {
    passed: failures.length === 0 && report.verdict !== "abandon",
    average,
    failures,
    abandonRecommended,
    abandonReason,
...
export function makeWatchabilityReleaseWorker(): WorkerDef {
  return {
    ...
    async execute(inputs, ctx): Promise<WorkerOutput> {
      ...
      if (!result.passed) {
        const disposition = result.abandonRecommended
          ? `ABANDON_TOPIC: ${result.abandonReason}`
          : "REVISE_SCRIPT";
        throw new Error(
          `watchability release blocked (${disposition}; attempt ${ctx.attemptNumber}): ${result.failures.join("; ")}`,
        );
      }
