/** fal.ai premium generated-video provider for RFC 0010 hero beats. */
...
export class FalVideoProvider {
  ...
  constructor(opts: FalVideoOptions = {}) {
    ...
    this.model = opts.model ?? process.env["FAL_VIDEO_MODEL"] ?? "fal-ai/kling-video/v2.5-turbo/pro/text-to-video";
  ...
  async generate(prompt: string, requestedDurationSec = 5): Promise<GeneratedVideo> {
    ...
    const submitRes = await this.fetchImpl(`${this.queueBaseUrl}/${this.model}`, {
      ...
      body: JSON.stringify({
        ...
        duration,
        aspect_ratio: "16:9",
        negative_prompt: "blur, distort, low quality, subtitles, text, watermark, logo",
        cfg_scale: 0.5,
      }),
    });
    if (!submitRes.ok) {
      throw new ProviderError(`${this.id} submit returned ${submitRes.status}: ${(await submitRes.text()).slice(0, 500)}`);
    }
    ...
    for (;;) {
      ...
      const status = await statusRes.json() as QueueStatus;
    ...
    const result = await resultRes.json() as QueueResult;
    const videoUrl = result.video?.url;
    if (!videoUrl) throw new ProviderError(`${this.id} result has no video URL`);
    const dl = await this.fetchImpl(videoUrl);
    if (!dl.ok) throw new ProviderError(`${this.id} video download returned ${dl.status}`);
    return {
