...
export const CREDENTIALS: CredentialSpec[] = [
  ...
  },
  {
    ...
    secret: false,
    placeholder: "24",
    help: "ON by default (every 24h) once deployed; discovers a topic and starts the illustrated-story production graph. Set to 0 to disable and start runs by hand only.",
    fallback: "24 (daily, auto-production on)",
  },
  {
    key: "SCHEDULE_PRODUCE_TIMEZONE",
    label: "Daily publish time zone",
    secret: false,
    placeholder: "Europe/Berlin",
    help: "IANA time zone name. The daily produce job targets SCHEDULE_PRODUCE_LOCAL_HOUR in this zone, resolved to the correct UTC hour fresh on every restart -- so the slot doesn't drift an hour across a DST change the way a fixed UTC hour would. Overridden entirely if SCHEDULE_PRODUCE_HOUR_UTC is set.",
    fallback: "Europe/Berlin",
  },
  {
    key: "SCHEDULE_PRODUCE_LOCAL_HOUR",
    label: "Daily publish local hour (0-23)",
    secret: false,
    placeholder: "21",
    help: "The local clock hour in SCHEDULE_PRODUCE_TIMEZONE the daily produce job targets, e.g. 21 for 9pm. Overridden entirely if SCHEDULE_PRODUCE_HOUR_UTC is set.",
    fallback: "21 (9pm)",
  },
  {
    key: "SCHEDULE_PRODUCE_HOUR_UTC",
    label: "Daily publish UTC hour override (0-23)",
    secret: false,
    placeholder: "",
    help: "Advanced: pin a literal UTC hour instead of resolving one from SCHEDULE_PRODUCE_TIMEZONE/SCHEDULE_PRODUCE_LOCAL_HOUR. A fixed UTC hour does not track DST -- it will drift an hour relative to any local-time target twice a year. Leave unset unless you specifically want a DST-independent slot.",
    fallback: "unset -- resolved from SCHEDULE_PRODUCE_TIMEZONE/SCHEDULE_PRODUCE_LOCAL_HOUR instead",
  },
  {
    key: "COMPOSE_URL",
