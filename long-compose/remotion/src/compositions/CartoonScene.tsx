...
function motionStyle(prop: ForegroundPropSpec, frame: number) {
...
function ForegroundPropOverlay({ prop }: { prop?: ForegroundPropSpec }) {
    ...
    const { trembleX, slideX, hoverY, pulse, scale } = motionStyle(prop, frame);
    ...
    if (type === "phone") {
        ...
        return (
            <div style={{ position: "absolute", left: x + trembleX + slideX, top: y + hoverY, width: 154, height: 250, borderRadius: 28, background: faceDown ? "#20242C" : "#111827", border: "8px solid #F8FAFC", boxShadow: glow ? `0 0 ${34 + pulse * 28}px rgba(80,190,255,0.58)` : "0 18px 38px rgba(0,0,0,0.28)", transform: `rotate(${prop.anchor === "hand" ? -8 : 5}deg) scale(${scale})`, zIndex: 7 }}>
                {!faceDown && <div style={{ position: "absolute", left: 14, top: 18, width: 126, height: 210, borderRadius: 18, background: "linear-gradient(180deg,#58C7FF,#1E3A8A)", boxShadow: glow ? "inset 0 0 24px rgba(255,255,255,0.22)" : "none" }} />}
                {String(prop.motion || "").toLowerCase() === "thumb-hover" && <div style={{ position: "absolute", left: -45, bottom: 35 + hoverY, width: 70, height: 44, borderRadius: 28, background: "#F2C7A5", border: "5px solid rgba(40,35,30,0.45)", transform: "rotate(14deg)", boxShadow: "0 8px 16px rgba(0,0,0,0.18)" }} />}
            </div>
        );
    }
    ...
    if (type === "keys") {
        return <div style={{ position: "absolute", left: x + trembleX, top: y + hoverY, width: 240, height: 130, transform: `rotate(-8deg) scale(${scale})`, zIndex: 7 }}><div style={{ position: "absolute", left: 12, top: 30, width: 72, height: 72, borderRadius: 72, border: "14px solid #FBBF24", boxShadow: "0 12px 24px rgba(0,0,0,0.22)" }} /><div style={{ position: "absolute", left: 74, top: 60, width: 142, height: 18, borderRadius: 10, background: "#F59E0B", boxShadow: "0 12px 24px rgba(0,0,0,0.18)" }} /><div style={{ position: "absolute", right: 20, top: 48, width: 22, height: 44, background: "#F59E0B" }} /><div style={{ position: "absolute", right: 55, top: 60, width: 18, height: 36, background: "#F59E0B" }} /></div>;
    }

    if (type === "route-map") {
        const delayed = /traffic|delay|red/.test(state);
        return <div style={{ position: "absolute", left: x + trembleX, top: y + hoverY, width: 360, height: 210, borderRadius: 24, background: "#EFF6FF", border: "8px solid #FFFFFF", boxShadow: "0 18px 42px rgba(0,0,0,0.25)", transform: `rotate(3deg) scale(${scale})`, zIndex: 7, overflow: "hidden" }}><div style={{ position: "absolute", left: 0, top: 86, width: 360, height: 18, background: "#BFDBFE" }} /><div style={{ position: "absolute", left: 58, top: 26, width: 34, height: 160, borderRadius: 24, background: "#BFDBFE" }} /><div style={{ position: "absolute", left: 64, top: 103, width: 230, height: 16, borderRadius: 12, background: delayed ? "#EF4444" : "#22C55E", boxShadow: delayed ? `0 0 ${18 + pulse * 20}px rgba(239,68,68,0.42)` : "0 0 18px rgba(34,197,94,0.28)" }} /><div style={{ position: "absolute", left: 274, top: 89, width: 42, height: 42, borderRadius: 42, background: delayed ? "#EF4444" : "#22C55E", border: "6px solid #FFFFFF" }} /></div>;
    }

    if (type === "calendar") {
        const buffered = /buffer/.test(state);
        return <div style={{ position: "absolute", left: x + trembleX, top: y + hoverY, width: 260, height: 210, borderRadius: 22, background: "#FFFFFF", border: "8px solid #20242C", boxShadow: "0 18px 40px rgba(0,0,0,0.24)", transform: `rotate(-3deg) scale(${scale})`, zIndex: 7, overflow: "hidden" }}><div style={{ height: 48, background: buffered ? "#22C55E" : "#2563EB" }} /><div style={{ position: "absolute", left: 28, top: 70, right: 28, height: 20, borderRadius: 10, background: "#CBD5E1" }} /><div style={{ position: "absolute", left: 28, top: 106, right: buffered ? 28 : 92, height: 20, borderRadius: 10, background: buffered ? "#22C55E" : "#CBD5E1" }} /><div style={{ position: "absolute", left: 28, top: 144, right: 64, height: 20, borderRadius: 10, background: "#CBD5E1" }} /></div>;
    }

    if (type === "door") {
        const open = /open|leaving/.test(state);
        return <div style={{ position: "absolute", left: x + trembleX, top: y + hoverY - 70, width: 170, height: 300, borderRadius: 10, background: open ? "#92400E" : "#78350F", border: "8px solid #451A03", boxShadow: open ? `0 0 ${28 + pulse * 22}px rgba(251,191,36,0.35)` : "0 18px 38px rgba(0,0,0,0.25)", transform: `perspective(360px) rotateY(${open ? -20 : 0}deg) scale(${scale})`, transformOrigin: "left center", zIndex: 7 }}><div style={{ position: "absolute", right: 18, top: 142, width: 18, height: 18, borderRadius: 18, background: "#FBBF24" }} /></div>;
    }

    if (type === "coffee" || type === "mug") {
        return <div style={{ position: "absolute", left: x + trembleX, top: y + hoverY, width: 160, height: 150, transform: `rotate(-3deg) scale(${scale})`, zIndex: 7 }}><div style={{ position: "absolute", left: 30, top: 42, width: 100, height: 92, borderRadius: "0 0 32px 32px", background: "#FFFFFF", border: "8px solid #20242C", boxShadow: "0 18px 38px rgba(0,0,0,0.22)" }} /><div style={{ position: "absolute", right: 8, top: 60, width: 46, height: 44, borderRadius: 44, border: "8px solid #20242C" }} /><div style={{ position: "absolute", left: 52, top: 20 + Math.sin(frame / 8) * 5, width: 16, height: 38, borderRadius: 16, background: "rgba(255,255,255,0.68)" }} /><div style={{ position: "absolute", left: 88, top: 14 + Math.sin(frame / 7) * 5, width: 14, height: 48, borderRadius: 14, background: "rgba(255,255,255,0.58)" }} /></div>;
    }

    if (type === "shoes" || type === "shoe") {
        return <div style={{ position: "absolute", left: x + trembleX, top: y + hoverY, width: 270, height: 130, transform: `rotate(-4deg) scale(${scale})`, zIndex: 7 }}><div style={{ position: "absolute", left: 10, top: 50, width: 118, height: 54, borderRadius: "48px 28px 18px 18px", background: "#334155", boxShadow: "0 14px 28px rgba(0,0,0,0.22)" }} /><div style={{ position: "absolute", left: 142, top: 54, width: 118, height: 54, borderRadius: "48px 28px 18px 18px", background: "#475569", boxShadow: "0 14px 28px rgba(0,0,0,0.22)" }} /><div style={{ position: "absolute", left: 28, top: 42, width: 62, height: 8, borderRadius: 8, background: "#F8FAFC" }} /><div style={{ position: "absolute", left: 160, top: 46, width: 62, height: 8, borderRadius: 8, background: "#F8FAFC" }} /></div>;
    }

    if (type === "laptop") {
        const glow = /glow|open|screen/.test(state);
        return <div style={{ position: "absolute", left: x + trembleX + slideX, top: y + hoverY, width: 300, height: 190, transform: `rotate(2deg) scale(${scale})`, zIndex: 7 }}><div style={{ position: "absolute", left: 42, top: 8, width: 216, height: 126, borderRadius: 16, background: glow ? "linear-gradient(180deg,#38BDF8,#1E3A8A)" : "#334155", border: "8px solid #0F172A", boxShadow: glow ? `0 0 ${24 + pulse * 24}px rgba(56,189,248,0.42)` : "0 14px 28px rgba(0,0,0,0.22)" }} /><div style={{ position: "absolute", left: 8, top: 130, width: 284, height: 34, borderRadius: "8px 8px 20px 20px", background: "#CBD5E1", border: "6px solid #0F172A" }} /></div>;
    }

    if (type === "bed" || type === "sheets") {
        return <div style={{ position: "absolute", left: x + trembleX, top: y + hoverY - 20, width: 330, height: 150, transform: `scale(${scale})`, zIndex: 7 }}><div style={{ position: "absolute", left: 0, top: 58, width: 330, height: 72, borderRadius: "26px 26px 18px 18px", background: "#93C5FD", border: "8px solid rgba(15,23,42,0.45)", boxShadow: "0 16px 34px rgba(0,0,0,0.20)" }} /><div style={{ position: "absolute", left: 22, top: 22, width: 112, height: 58, borderRadius: 20, background: "#F8FAFC", border: "6px solid rgba(15,23,42,0.26)" }} /><div style={{ position: "absolute", right: 18, bottom: 6, width: 46, height: 70, borderRadius: 14, background: "#1E293B" }} /></div>;
    }
...
function NonCardEventEffect({ event }: { event?: VisualEventSpec }) {
    const frame = useCurrentFrame();
...
function VisualEventOverlay({ event }: { event?: VisualEventSpec }) {
    return <><NonCardEventEffect event={event} /><ForegroundPropOverlay prop={event?.foregroundProp} /></>;
}
