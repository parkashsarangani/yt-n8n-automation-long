...
// soda-container and soda-scale are the EXACT scenes from a real render
// (run_bd3de54c, a carbonation episode) that failed production motion QA --
// pulled verbatim from that run's asset_manifest, not invented fixtures.
// soda-container authors "rotate"/"exit", neither of which is in
// container-object's own vocabulary (["drop","enter"],["rise"],["sink"]);
// soda-scale authors "compare" on TWO separate windows, which used to make
// the first (earlier-saturating) window mask the second one's later
// progress via Math.max. Both rendered as fully static for their back half
// before shared.tsx's actionSlot/matchedProgress fix. Kept alongside ice/
// onion/million-billion-trillion (whose hand-picked verbs happened to
// already match, which is exactly why they never caught this) so this
// specific regression can't silently return.
test("ice, onion, scale, and real-production benchmarks render specific progressing semantic scenes",{timeout:420000},()=>{const dir=fs.mkdtempSync(path.join(os.tmpdir(),"semantic-benchmarks-")),remotion=path.join(__dirname,"../remotion");try{run(process.execPath,["scripts/render-semantic-benchmarks.mjs",dir],{cwd:remotion});const manifest=JSON.parse(fs.readFileSync(path.join(dir,"manifest.json"),"utf8"));assert.deepEqual(manifest.fixtures,["ice","onion","million-billion-trillion","soda-container","soda-scale"]);for(const id of manifest.fixtures){const early=path.join(dir,`${id}-20.png`),middle=path.join(dir,`${id}-55.png`),late=path.join(dir,`${id}-85.png`);assert.ok(similarity(early,middle)<.9995,`${id} has no 20%-55% progression`);assert.ok(similarity(middle,late)<.9995,`${id} has no 55%-85% progression`)}assert.ok(similarity(path.join(dir,"ice-85.png"),path.join(dir,"onion-85.png"))<.97,"ice and onion silhouettes are too reusable");assert.ok(similarity(path.join(dir,"onion-85.png"),path.join(dir,"million-billion-trillion-85.png"))<.97,"onion and scale silhouettes are too reusable")}finally{fs.rmSync(dir,{recursive:true,force:true})}});
