...
async function buildStockTrack(scenes, durations, shots, directory, {ffmpeg, exec, warn = console.warn}) {
  ...
  for (let i=0;i<scenes.length;i++) {
    ...
    if (shot) {
      ...
      } catch { warn(`Stock scene ${scenes[i].scene_index} could not be decoded; using background`); }
    }
  ...
  await fs.writeFile(list,segments.map(file=>`file '${file.replace(/\\/g,'/').replace(/'/g,"'\\''")}'`).join('\n'));
