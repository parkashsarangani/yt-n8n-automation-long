...
- The long-form programme stays 1920×1080. After script approval, a bounded visual plan combines a few text-free continuity-prompted artwork beats with deterministic evidence, comparison, timeline and payoff overlays. Clean colour fallback is used when artwork billing/access is unavailable.
- Caption cards, role labels and scene counters are retained as separate layers. Short phrases use ElevenLabs character timing when valid and text-matching; old artifacts use proportional timing within measured scene durations. The package title is a persistent heading.
...
Not verified in development: paid voice auditions, fal generation, production deployment, or a full live episode. A successful prior render retains its visual plan and scene artwork blobs for explicit rerenders; matching artwork is reused by continuity reference, while an image generated during a render that fails before artifact creation may be generated again on retry.
